...
**Args:** Supports `--headless` or `-H` for autonomous execution. Named tasks: `--headless:discover` (tool check), `--headless:status` (campaign overview).
...
|------------|------------|----------------|
| Strategist | `paw-cra-strategist` | Research, scripts, copy, SEO content |
